...
// ── PrintBridge config ────────────────────────────────────────────────────────
const PRINT_SERVER = "https://imprimante-system-production.up.railway.app";
...
const NAV_ITEMS = [
    { label: "Tableau de bord", icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><rect x="3" y="3" width="7" height="7" rx="1" /><rect x="14" y="3" width="7" height="7" rx="1" /><rect x="3" y="14" width="7" height="7" rx="1" /><rect x="14" y="14" width="7" height="7" rx="1" /></svg> },
    { label: "Dossiers", icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z" /></svg> },
    { label: "Affaires", icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 7H4a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2z" /><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16" /></svg> },
    { label: "Finances", icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="12" y1="1" x2="12" y2="23" /><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6" /></svg> },
];
const AFFAIRES = [
...
const EMPTY_FORM = { dossierName: "Plan de Bornage", fileType: "PDF", clientName: "", operator: "", dateEmission: new Date().toISOString().split("T")[0], dateLivraison: "", montant: "", status: "En attente", note: "" };
...
// ── PDF Generator (pure JS, no lib) ──────────────────────────────────────────
// Builds a minimal valid PDF and returns a Blob
function buildDossierPDF(d) {
    const title = `${d.dossierName} — ${d.clientName}`;
    const lines = [
        `Dossier       : ${d.dossierName} (${d.fileType})`,
        `Client        : ${d.clientName}`,
        `Operateur     : ${d.operator}`,
        `Date emission : ${fmtDate(d.dateEmission)}`,
        `Date livraison: ${fmtDate(d.dateLivraison)}`,
        `Montant       : ${fmtMontant(d.montant)}`,
        `Status        : ${d.status}`,
        d.note ? `Note          : ${d.note}` : null,
    ].filter(Boolean);

    // Build PDF manually (subset of PDF 1.4 spec)
    const escStr = s => s.replace(/\\/g, "\\\\").replace(/\(/g, "\\(").replace(/\)/g, "\\)");

    let objs = [];
    const obj = (id, dict) => { objs[id] = dict; };
    ...
    let stream = "";
    stream += "BT\n";
    stream += `/F1 16 Tf\n${margin} ${y} Td\n(${escStr("GeoTech Ingé — Fiche Dossier")}) Tj\n`;
    y -= 10;
    stream += `/F1 10 Tf\n0 -20 Td\n(${escStr("─".repeat(70))}) Tj\n`;
    y -= 20;
    stream += `0 -8 Td\n`;
    ...
    stream += `\n0 -30 Td\n/F1 9 Tf\n(Genere le ${new Date().toLocaleDateString("fr-FR")}) Tj\n`;
    stream += "ET\n";
    ...
    const writeObj = (id, dict, streamData) => {
        offsets[id] = pdf.length;
        pdf += `${id} 0 obj\n`;
        if (streamData) {
            pdf += `<< ${dict} /Length ${streamData.length} >>\nstream\n`;
            pdf += stream;
            pdf += `\nendstream\n`;
        } else {
            pdf += `<< ${dict} >>\n`;
    ...
    writeObj(4, "", true); // stream content
...
// ── PrintBridge Upload ────────────────────────────────────────────────────────
async function uploadToPrintBridge(blob, filename) {
    const formData = new FormData();
    formData.append("file", blob, filename);
    const res = await fetch(`${PRINT_SERVER}/upload`, { method: "POST", body: formData });
    if (!res.ok) { const e = await res.json(); throw new Error(e.error || "Upload failed"); }
    return res.json();
}

// ── Print Status Toast ────────────────────────────────────────────────────────
function PrintToast({ state, filename, onClose }) {
    useEffect(() => { if (state === "success") { const t = setTimeout(onClose, 4000); return () => clearTimeout(t); } }, [state]);
    const cfg = {
        loading: {
            bg: "#E3F2FD", color: "#0277BD", border: "#BBDEFB", icon: (
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#0277BD" strokeWidth="2" style={{ animation: "spin 1s linear infinite" }}><circle cx="12" cy="12" r="10" strokeDasharray="40" strokeDashoffset="10" /></svg>
            ), msg: "Envoi vers l'imprimante…"
        },
        success: {
            bg: "#E8F5E9", color: "#2E7D32", border: "#C8E6C9", icon: (
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#2E7D32" strokeWidth="2.5"><polyline points="20 6 9 17 4 12" /></svg>
            ), msg: `✓ "${filename}" envoyé à l'imprimante !`
        },
        error: {
            bg: "#FFF3E0", color: "#E65100", border: "#FFCC80", icon: (
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#E65100" strokeWidth="2"><circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="12" /><line x1="12" y1="16" x2="12.01" y2="16" /></svg>
            ), msg: "Erreur — serveur PrintBridge inaccessible"
        },
    };
    const c = cfg[state];
    return (
        <div style={{ position: "fixed", bottom: 24, right: 24, zIndex: 500, background: c.bg, border: `1.5px solid ${c.border}`, borderRadius: 12, padding: "12px 18px", display: "flex", alignItems: "center", gap: 12, boxShadow: "0 8px 30px #00000018", animation: "slideUp .3s ease", minWidth: 280 }}>
            {c.icon}
            <span style={{ fontSize: 13, fontWeight: 600, color: c.color, flex: 1 }}>{c.msg}</span>
            {state !== "loading" && (
                <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: c.color, fontSize: 16, lineHeight: 1, padding: 2 }}>×</button>
            )}
        </div>
    );
}
...
// ── Print Queue Panel ──────────────────────────────────────────────────────────
function PrintQueuePanel({ onClose }) {
    ...
    const fetchQueue = async () => {
        ...
        try {
            ...
            setQueue(data);
        } catch { setErr("Impossible de contacter PrintBridge — vérifiez que le serveur tourne."); }
        finally { setLoading(false); }
    };

    const deleteFile = async (name) => {
        try {
            await fetch(`${PRINT_SERVER}/delete/${encodeURIComponent(name)}`, { method: "DELETE" });
            fetchQueue();
    ...
    return (
        <div className="gt-overlay" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
            <div className="gt-modal" style={{ maxWidth: 480 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
                    <div style={{ width: 40, height: 40, background: "#F3F0FF", borderRadius: 11, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    ...
                    <div>
                        <h3 style={{ margin: 0, fontSize: 16, fontWeight: 800, color: "#212121" }}>File d'impression</h3>
                        <p style={{ margin: 0, fontSize: 12, color: "#9E9E9E" }}>PrintBridge — {PRINT_SERVER}</p>
                    </div>
                    <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
                        <button onClick={fetchQueue} title="Rafraîchir" style={{ background: "#F5F5F5", border: "none", width: 32, height: 32, borderRadius: 8, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "#616161" }}>
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="23 4 23 10 17 10" /><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10" /></svg>
                        </button>
                        <button onClick={onClose} style={{ background: "#F5F5F5", border: "none", width: 32, height: 32, borderRadius: 8, cursor: "pointer", fontSize: 16, color: "#9E9E9E" }}>×</button>
                    </div>
                </div>

                {loading && (
                    <div style={{ textAlign: "center", padding: "32px", color: "#9E9E9E", fontSize: 13 }}>
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#9E9E9E" strokeWidth="2" style={{ animation: "spin 1s linear infinite", display: "block", margin: "0 auto 12px" }}><circle cx="12" cy="12" r="10" strokeDasharray="40" strokeDashoffset="10" /></svg>
                        Connexion au serveur…
                    </div>
                )}
                {err && (
                    <div style={{ background: "#FFF3E0", border: "1px solid #FFCC80", borderRadius: 10, padding: "14px 16px", fontSize: 13, color: "#E65100" }}>
                        ⚠️ {err}
                        <br /><span style={{ fontSize: 11, color: "#BF360C", marginTop: 4, display: "block" }}>Assurez-vous que <code>node index.js</code> tourne sur le port 3001.</span>
                    </div>
                )}
                {!loading && !err && queue && (
                    <>
                        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
                            <div style={{ background: queue.queue === 0 ? "#E8F5E9" : "#FFF3E0", color: queue.queue === 0 ? "#2E7D32" : "#E65100", border: `1px solid ${queue.queue === 0 ? "#C8E6C9" : "#FFCC80"}`, borderRadius: 20, padding: "3px 12px", fontSize: 12, fontWeight: 700 }}>
                                {queue.queue} fichier{queue.queue !== 1 ? "s" : ""} en attente
                            </div>
                            <span style={{ fontSize: 11, color: "#BDBDBD" }}>Mis à jour: {new Date(queue.serverTime).toLocaleTimeString("fr-FR")}</span>
                        </div>
                        {queue.files.length === 0 ? (
                            <div style={{ textAlign: "center", padding: "28px", color: "#BDBDBD", fontSize: 13 }}>
                                <div style={{ fontSize: 32, marginBottom: 8 }}>🖨️</div>
                                File vide — aucun job en attente
                            </div>
                        ) : (
                            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                                {queue.files.map((f, i) => (
                                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, background: "#F8F9FA", borderRadius: 10, padding: "10px 14px", border: "1px solid #EEEEEE" }}>
                                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#5C35C9" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /></svg>
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            <div style={{ fontSize: 13, fontWeight: 700, color: "#212121", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{f.name}</div>
                                            <div style={{ fontSize: 11, color: "#9E9E9E" }}>{(f.size / 1024).toFixed(1)} KB — {new Date(f.queued).toLocaleString("fr-FR")}</div>
                                        </div>
                                        <a href={`${PRINT_SERVER}/download/${encodeURIComponent(f.name)}`} target="_blank" rel="noreferrer"
                                            style={{ background: "#5C35C9", color: "white", border: "none", padding: "5px 12px", borderRadius: 7, fontSize: 12, fontWeight: 600, textDecoration: "none", cursor: "pointer" }}>
                                            Télécharger
                                        </a>
                                        <button onClick={() => deleteFile(f.name)} title="Supprimer" style={{ background: "none", border: "none", cursor: "pointer", color: "#BDBDBD", padding: 4, borderRadius: 6, display: "flex" }}
                                            onMouseEnter={e => e.currentTarget.style.color = "#E53935"} onMouseLeave={e => e.currentTarget.style.color = "#BDBDBD"}>
    ...
    return (
        <div style={{ background: "white", borderRadius: 14, padding: "18px 20px", boxShadow: "0 2px 12px #0000000d", border: "1px solid #F0F0F0" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                <button onClick={() => setCur(c => { const d = new Date(c.year, c.month - 1); return { year: d.getFullYear(), month: d.getMonth() }; })} style={{ background: "none", border: "none", cursor: "pointer", color: "#9E9E9E", fontSize: 16, padding: "2px 6px" }}>‹</button>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: "2px 0", textAlign: "center" }}>
                {["SAN", "MON", "TUE", "WED", "THU", "FRI", "SAT"].map(d => <div key={d} style={{ fontSize: 9, fontWeight: 700, color: "#9E9E9E", paddingBottom: 6 }}>{d}</div>)}
                {cells.map((d, i) => (
                    <div key={i} style={{
                        fontSize: 11, padding: "5px 2px", borderRadius: 20, cursor: d ? "pointer" : "default",
                        background: isThisMonth && d === now.getDate() ? "#5C35C9" : "transparent",
                        color: d === null ? "transparent" : isThisMonth && d === now.getDate() ? "white" : "#424242",
                        fontWeight: isThisMonth && d === now.getDate() ? 700 : 400,
                    }}>{d || ""}</div>
                ))}
            </div>
        </div>
    ...
    return (
        <div style={{ background: "white", borderRadius: 14, padding: "18px 20px", boxShadow: "0 2px 12px #0000000d", border: "1px solid #F0F0F0", marginTop: 16 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                ...
                <div style={{ background: "#5C35C9", color: "white", borderRadius: 20, fontSize: 11, fontWeight: 700, padding: "2px 8px" }}>{affaires.length}</div>
            </div>
            {affaires.map((a, i) => (
                <div key={i} style={{ borderBottom: i < affaires.length - 1 ? "1px solid #F5F5F5" : "none", paddingBottom: i < affaires.length - 1 ? 14 : 0, marginBottom: i < affaires.length - 1 ? 14 : 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: "#9E9E9E", marginBottom: 4 }}>⊘ {a.location}</div>
...
// ── Row Menu ──────────────────────────────────────────────────────────────────
function RowMenu({ onEdit, onDelete, onWhatsApp, onPrint }) {
    ...
    return (
        <div ref={ref} style={{ position: "relative", display: "inline-block" }}>
            ...
            {open && (
                <div style={{ position: "absolute", right: 0, top: 28, zIndex: 300, background: "white", border: "1px solid #EEEEEE", borderRadius: 10, padding: 6, boxShadow: "0 8px 30px #00000015", minWidth: 185, animation: "menuPop .15s ease" }}>
                    ...
                    <button style={btn} onMouseEnter={e => e.currentTarget.style.background = "#F3F0FF"} onMouseLeave={e => e.currentTarget.style.background = "transparent"} onClick={() => { setOpen(false); onPrint(); }}>
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#5C35C9" strokeWidth="2"><polyline points="6 9 6 2 18 2 18 9" /><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2" /><rect x="6" y="14" width="12" height="8" /></svg>
                        Imprimer (PrintBridge)
                    </button>
        ...
        <div className="gt-overlay" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
            <div className="gt-modal">
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
                    ...
                    <div><h3 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: "#212121" }}>Envoyer sur WhatsApp</h3><p style={{ margin: 0, fontSize: 12, color: "#9E9E9E" }}>{dossier.dossierName} — {dossier.clientName}</p></div>
                ...
                <div style={{ background: "#F8F9FA", borderRadius: 10, padding: "12px 14px", marginBottom: 16, fontSize: 12, color: "#616161", lineHeight: 1.9 }}>
                    <div><b>📁 Dossier :</b> {dossier.dossierName} ({dossier.fileType})</div>
                    <div><b>👤 Client :</b> {dossier.clientName}</div>
                    <div><b>📅 Émission :</b> {fmtDate(dossier.dateEmission)}</div>
                    <div><b>🚚 Livraison :</b> {fmtDate(dossier.dateLivraison)}</div>
                    <div><b>💰 Montant :</b> {fmtMontant(dossier.montant)}</div>
                    {dossier.note && <div><b>📝 Note :</b> {dossier.note}</div>}
                </div>
                <label style={{ fontSize: 12, fontWeight: 600, color: "#616161", display: "block", marginBottom: 6 }}>Numéro WhatsApp</label>
                <input className="gt-input" placeholder="212600000000" value={phone} onChange={e => { setPhone(e.target.value); setErr(""); }} />
                ...
                <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                    ...
                    <button onClick={send} style={{ background: "#25D366", color: "white", border: "none", padding: "9px 20px", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>Envoyer</button>
                </div>
            </div>
        </div>
    );
    ...
    return (
        <div className="gt-overlay" onClick={e => { if (e.target === e.currentTarget) onCancel(); }}>
            <div className="gt-modal" style={{ maxWidth: 380 }}>
                ...
                <p style={{ color: "#757575", fontSize: 13, margin: "0 0 22px", lineHeight: 1.6 }}>Vous allez supprimer <strong style={{ color: "#212121" }}>« {name} »</strong>. Action irréversible.</p>
                <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
        ...
        <div className="gt-overlay" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
            <div className="gt-modal">
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 22 }}>
                    <h2 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: "#212121" }}>{editingId !== null ? "Modifier le Dossier" : "Ajouter un Dossier"}</h2>
                ...
                <div style={{ display: "grid", gap: 14 }}>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 120px", gap: 12 }}>
                        <div className="gt-field">
                            ...
                            <input className={`gt-input${errors.dossierName ? " gt-err" : ""}`} value={form.dossierName} onChange={e => onChange("dossierName", e.target.value)} />
                    ...
                    <div className="gt-field">
                        ...
                        <input className={`gt-input${errors.clientName ? " gt-err" : ""}`} value={form.clientName} onChange={e => onChange("clientName", e.target.value)} placeholder="ex: Riad Shemsi" />
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                        ...
                        <div className="gt-field">
                            ...
                            <input className="gt-input" type="number" placeholder="ex: 5000" value={form.montant} onChange={e => onChange("montant", e.target.value)} />
                        </div>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                        <div className="gt-field">
                    ...
                    <div className="gt-field">
                        ...
                        <textarea className="gt-input" rows={3} value={form.note} onChange={e => onChange("note", e.target.value)} style={{ resize: "vertical" }} />
                    </div>
                    {editingId !== null ? (
                        <div className="gt-field">
                    ...
                    <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 4 }}>
                        <button className="gt-btn-sec" onClick={onClose}>Annuler</button>
                        <button onClick={onSubmit} className="gt-btn-pri">{editingId !== null ? "Enregistrer" : "Ajouter un Dossier"}</button>
                    </div>
                </div>
            </div>
        </div>
...
export default function App() {
    ...
    const [printToast, setPrintToast] = useState(null); // { state, filename }
    const [queueCount, setQueueCount] = useState(null);

    // Poll queue count every 10s
    useEffect(() => {
        const poll = async () => {
            try { const r = await fetch(`${PRINT_SERVER}/status`); const d = await r.json(); setQueueCount(d.queue); } catch { setQueueCount(null); }
        };
        poll();
    ...
    const handleSubmit = () => {
        if (!validate()) return;
        if (editingId !== null) { setDossiers(p => p.map(d => d.id === editingId ? { ...d, ...form } : d)); setAnimRow(editingId); }
        else { const id = Date.now(); setDossiers(p => [...p, { id, ...form, status: "En attente" }]); setAnimRow(id); }
        setShowForm(false);
    ...
    const handlePrint = async (d) => {
        const safeName = `${d.dossierName.replace(/[^a-zA-Z0-9]/g, "_")}_${d.clientName.replace(/[^a-zA-Z0-9]/g, "_")}.pdf`;
        setPrintToast({ state: "loading", filename: safeName });
        try {
            const blob = buildDossierPDF(d);
            await uploadToPrintBridge(blob, safeName);
            setPrintToast({ state: "success", filename: safeName });
            // refresh queue count
            try { const r = await fetch(`${PRINT_SERVER}/status`); const data = await r.json(); setQueueCount(data.queue); } catch { }
        } catch (err) {
            console.error(err);
            setPrintToast({ state: "error", filename: safeName });
        }
    };
    ...
    return (
        ...
        * { box-sizing:border-box; }
        ...
        .gt-btn-sec { background:#F5F5F5;color:#616161;border:none;padding:9px 20px;border-radius:9px;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit; }
      ...
      `}</style>
            ...
            <aside style={{ width: 220, background: "white", borderRight: "1px solid #F0F0F0", display: "flex", flexDirection: "column", flexShrink: 0, padding: "20px 12px" }}>
                ...
                <nav style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                    {NAV_ITEMS.map((item, i) => (
                        <button key={i} className={`nav-item${activeNav === i ? " nav-active" : ""}`} onClick={() => setActiveNav(i)}>{item.icon}{item.label}</button>
                    ))}
                </nav>
                {/* PrintBridge status in sidebar */}
                <div style={{ marginTop: "auto", padding: "12px 10px", background: queueCount === null ? "#FFF3E0" : queueCount === 0 ? "#E8F5E9" : "#F3F0FF", borderRadius: 10, border: `1px solid ${queueCount === null ? "#FFCC80" : queueCount === 0 ? "#C8E6C9" : "#D1C4E9"}`, cursor: "pointer" }} onClick={() => setShowQueue(true)}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={queueCount === null ? "#E65100" : queueCount === 0 ? "#2E7D32" : "#5C35C9"} strokeWidth="2"><polyline points="6 9 6 2 18 2 18 9" /><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2" /><rect x="6" y="14" width="12" height="8" /></svg>
                        <div>
                            <div style={{ fontSize: 11, fontWeight: 700, color: queueCount === null ? "#E65100" : queueCount === 0 ? "#2E7D32" : "#5C35C9" }}>
                                {queueCount === null ? "Hors ligne" : queueCount === 0 ? "Prête" : "En attente"}
                            </div>
                            <div style={{ fontSize: 10, color: "#9E9E9E" }}>
                                {queueCount === null ? "PrintBridge" : queueCount === 0 ? "File vide" : `${queueCount} job${queueCount > 1 ? "s" : ""}`}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
                ...
                <header style={{ background: "white", borderBottom: "1px solid #F0F0F0", height: 60, display: "flex", alignItems: "center", padding: "0 24px", gap: 16, flexShrink: 0 }}>
                    ...
                    <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 14 }}>
                        {/* Queue badge button */}
                        <button onClick={() => setShowQueue(true)} title="File d'impression" style={{ position: "relative", background: queueCount === null ? "#FFF3E0" : queueCount === 0 ? "#F5F5F5" : "#F3F0FF", border: "none", width: 36, height: 36, borderRadius: 10, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={queueCount === null ? "#E65100" : queueCount === 0 ? "#9E9E9E" : "#5C35C9"} strokeWidth="2"><polyline points="6 9 6 2 18 2 18 9" /><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2" /><rect x="6" y="14" width="12" height="8" /></svg>
                            {queueCount !== null && queueCount > 0 && (
                                <div style={{ position: "absolute", top: -4, right: -4, background: "#5C35C9", color: "white", borderRadius: 20, fontSize: 9, fontWeight: 800, padding: "1px 5px", minWidth: 16, textAlign: "center" }}>{queueCount}</div>
                            )}
                            {queueCount === null && (
                                <div style={{ position: "absolute", top: -3, right: -3, width: 10, height: 10, background: "#E65100", borderRadius: "50%", border: "2px solid white" }} />
                            )}
                        </button>
                    ...
                    <div style={{ display: "flex", gap: 24, alignItems: "flex-start" }}>
                        <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 16, marginBottom: 28 }}>
                                {[
                                    { icon: "⏰", label: "dues", value: enAttente, sub: "En attente", bg: "#FFF3E0" },
                                    { icon: "📄", label: "en cours", value: enCours, sub: "En cours", bg: "#E3F2FD" },
                                    { icon: "✅", label: "Livré", value: livre, sub: "Payée", bg: "#E8F5E9" },
                                    { icon: "💲", label: "Total dossier payé", value: totalPaye.toLocaleString(), sub: "DH", bg: "#E8F5E9", big: true },
                                ].map((c, i) => (
                            ...
                            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                                ...
                                <button className="gt-btn-pri" onClick={openNew} style={{ fontSize: 13, padding: "9px 20px", borderRadius: 10 }}>+ Ajouter un Dossier</button>
                            </div>
                                            ...
                                            {dossiers.length === 0 ? (
                                                ...
                                                return (
                                                    <tr key={d.id} className={`tr-row${animRow === d.id ? " tr-new" : ""}`} style={{ background: isSel ? "#F3F0FF" : "white" }}>
                                                        ...
                                                        {/* ── Print button — calls PrintBridge ── */}
                                                        <td>
                                                            <button
                                                                title="Imprimer via PrintBridge"
                                                                onClick={() => handlePrint(d)}
                                                                style={{ background: "none", border: "none", cursor: "pointer", padding: 4, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", transition: "background .15s" }}
            ...
            {showQueue && <PrintQueuePanel onClose={() => setShowQueue(false)} />}

            {/* Print toast */}
            {printToast && <PrintToast state={printToast.state} filename={printToast.filename} onClose={() => setPrintToast(null)} />}
        </div>
    );
}